...
exports.addModel = asyncHandler(async (req, res) => {
    const { modelName, backCoversCount, aluminumSheetsCount } = req.body;
    ...
    const brand = await phoneBrandService.addModelToBrand(req.params.id, {
        modelName,
        backCoversCount: backCoversCount || 0,
        aluminumSheetsCount: aluminumSheetsCount || 0
    });
...
/**
 * @desc Update model in brand
 * @route PUT /api/phone-brands/:id/models/:modelId
 * @access Private/Admin
 */
exports.updateModel = asyncHandler(async (req, res) => {
    const { modelName, backCoversCount, aluminumSheetsCount } = req.body;

    const brand = await phoneBrandService.updateModelInBrand(
        req.params.id,
        req.params.modelId,
        {
            modelName,
