...
const ithinkRoutes = require("./routes/ithink.routes");
...
// CORS - Allow frontend to access backend (including local LAN testing on phone)
...
// Debug middleware to log all incoming request
...
app.use("/api/ithink",ithinkRoutes);
...
process.on('unhandledRejection', (error) => {
  ...
  // Don't shutdown for every unhandled rejection - just log it
  // Only shutdown for critical errors (database connection, etc.)
  if (error.message && (error.message.includes('ECONNREFUSED') || error.message.includes('MongoDB'))) {
    console.error('Critical error detected. Starting graceful shutdown...');
